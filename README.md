...
import 410828 bytes (20000 facts)                                  0.03
insert 10 facts                                                    0.04
query 10 times w/txn                                               2.32
query 10 times w/o txn                                             0.07
modify 10 attrs w/txn                                              1.93
delete 10 facts w/txn                                              0.95
...
(and (eq what 'issue-was-closed') (exists... -> 200                1.16
...
(eq id (agg (always) (max id)))                                    2.56
(join "c<=cost,b<=bar" (eq id (agg (always) (ma...                 3.79
(and (eq what "foo") (join "w<=what" (and (eq i...                 6.86
delete!                                                            0.53
(and (eq issue *) (eq repository *) (eq what '*') (eq where '*'))  2.65
Taped.append() x50000                                              0.14
Taped.each() x125                                                  1.48
Taped.delete_if() x375                                             0.81
50000 facts: read-only txn (no copy needed)                       15.83
50000 facts: rollback txn (no copy needed)                        15.83
50000 facts: insert in txn (copy triggered)                        7.17
50000 facts: modify in txn (copy triggered)                       45.17
100000 facts: read-only txn (no copy needed)                      30.27
100000 facts: rollback txn (no copy needed)                       30.10
100000 facts: insert in txn (copy triggered)                      14.03
100000 facts: modify in txn (copy triggered)                      92.56
```

The results were calculated in [this GHA job][benchmark-gha]
on 2025-12-07 at 14:49,
on Linux with 4 CPUs.
<!-- benchmark_end -->

[benchmark-gha]: https://github.com/volodya-lombrozo/factbase/actions/runs/20005851262
